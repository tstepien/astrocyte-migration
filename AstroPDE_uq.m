...
% % % [thickness_ret,~,~,~] = thick_rad(t,x * u(4)); %tracy removed
[thickness_ret,~,~,~] = thick_rad(t,x * real(u(4)));
...
% % % xgrid = 1 + (nxpts - 1) * x * u(4) / rmax; %tracy removed
xgrid = 1 + (nxpts - 1) * x * real(u(4)) / rmax;
