function [c1_new,c2_new,eta_fraction] = cellpops_implicit_sigmoid(j,c1_old,c2_old,p1,p2,...
    dt,r,kappa,mu,alpha1,alpha2,beta,gamma1,gamma2,cmin,rbar,ce,tcurr,a,n)
...
maindiag21 = [-theta2_4 , -theta2_2 , 1];%0];
lowerdiag21 = [theta2_3 , 0];%theta6];

upperdiag22 = [theta2_4 , theta2_1];
maindiag22 = [1-theta2_4 , 1-theta2_2 , 0];%theta5];
lowerdiag22 = [theta2_3 , 0];%theta6];
...
% eta_fraction = min((1-exp(-beta*p2(j+1)*(tcurr+dt))) , 1);
% bc(end) = eta_fraction*ce;
% eta_fraction = 1/(1+beta*p2(j+1)*dt);
% bc(end) = ce - c1_old(j)/(1+beta*p2(j+1)*dt);
% eta_fraction = 1/(1-dt*(alpha1*p1(j+1)-beta*p2(j+1)-gamma1));
% bc(end) = ce - c1_old(j)/(1-dt*(alpha1*p1(j+1)-beta*p2(j+1)-gamma1));
% eta_fraction = 1/(1-dt*(alpha2*p1(j+1)-beta*p2(j+1)-gamma2));
% bc(end) = min( ( c2_old(j)+dt*beta*p2(j+1)*ce )...
%     /( 1-dt*(alpha2*p1(j+1)-beta*p2(j+1)-gamma2) ) , ce);
eta_fraction = sigmoidal(tcurr+dt,ce,a,n)/ce;
bc(end) = sigmoidal(tcurr+dt,ce,a,n);
